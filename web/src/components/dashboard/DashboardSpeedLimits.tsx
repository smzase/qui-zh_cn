...
export function DashboardSpeedLimits({
  ...
  const formatLimit = (limit: number) => limit === 0 ? t("common.unlimited") : formatSpeedWithUnit(limit * 1024, speedUnit)
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-muted-foreground">{t("common.download")}</span>
            </div>
            <p className="text-lg font-semibold">
              {formatLimit(preferences?.dl_limit || 0)}
            </p>
            <p className="text-xs text-muted-foreground">
              {t("common.current")}: {formatSpeedWithUnit(currentDownloadSpeed, speedUnit)}
            </p>
          </div>
          <div className="space-y-1">
            <div className="flex items-center gap-2">
            ...
            <p className="text-xs text-muted-foreground">
              {t("common.current")}: {formatSpeedWithUnit(currentUploadSpeed, speedUnit)}
            </p>
          </div>
        </div>
      </CardContent>
